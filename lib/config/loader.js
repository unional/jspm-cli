...
LoaderConfig.prototype.ensureRegistry = function(registryName) {
  ...
  if (!this.file.getValue(['paths', '*:*'], 'string'))
    this.file.setValue(['paths', '*:*'], 'jspm_packages/*/*');
};
...
LoaderConfigFile.prototype = Object.create(ConfigFile.prototype);
...
// NB finish this
LoaderConfig.prototype.prompt = function(promptType) {
  var self = this;

  var configPath = path.relative(config.pjson.baseURL, config.pjson.configFiles.jspm);
  var libPath = path.relative(config.pjson.baseURL, config.pjson.directories.lib);

  return ui.input('%' + configPath + ' baseURL% (optional, delete to remove)', self.baseURL || '/', {
    info: 'Enter the browser baseURL.\n\nThis is the SystemJS baseURL in the browser corresponding to the directories.baseURL folder on the server. This can also be optionally configured outside of the jspm config.js file.',
    edit: (self.baseURL || '/') == '/'
  })
  .then(function(baseURL) {
    self.baseURL = baseURL;

    return ui.input('%' + configPath + ' package format (esm, cjs, amd)%', self.format || 'esm', {
  ...
  .then(function(format) {
    ...
    if (promptType == 'custom')
      return ui.input('%' + configPath + ' package main%', self.main || (self.name || 'app' + '.js'), {
