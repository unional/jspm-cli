...
function upgradePackageConfig(packageConfig) {
  ...
  if (packageConfig.shim) {
    ...
    for (var s in packageConfig.shim) {
      ...
      if (packageConfig.modules[s + '.js'])
        continue;
      
      var curShim = packageConfig.shim[s];
      var curMeta = packageConfig.modules[s + '.js'] = {};
...
exports.download = function(pkg, options, installDeps, installPeerDeps) {
  ...
  .then(function(dirExists) {
    ...
    .then(function(fresh) {
      if (fresh && config.deps[pkg.exactName]) {
      ...
      var cacheDir = path.resolve(HOME, '.jspm', 'packages', pkg.registry, pkg.exactPackage);
...
function derivePackageConfig(pkg, packageConfig, override) {
  ...
  if (override)
    extend(packageConfig.jspm, override);
...
var loaderConfigProperties = ['baseDir', 'defaultExtension', 'format', 'modules', 'map', 'main'];
